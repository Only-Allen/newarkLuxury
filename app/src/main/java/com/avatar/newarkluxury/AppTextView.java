 ...
 */
public class AppTextView extends TextView implements LanguageChangeableView {
    ...
    private void init (Context context,AttributeSet attributeSet) {
        if (attributeSet!=null) {
            ...
            if (!(hintValue==null || hintValue.length()<2)) {
                hintId = string2int(hintValue.substring(1,hintValue.length()));
            }
        }
    }
    ...
    public int string2int (String str,int def) {
        try {
